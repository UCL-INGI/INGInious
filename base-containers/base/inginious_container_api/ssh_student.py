...
def ssh_student(setup_script=None, memory_limit=0, hard_time_limit=0, time_limit=0, container=None, run_as_root=False, closure_script=None):
    """
    ...
    :param closure_script: command to be ran (as a string, with parameters) in the student container after the student closed the ssh session.
    """
    return inginious_container_api.run_student.run_student(cmd=setup_script, container=container, time_limit=time_limit,
                ...
                signal_handler_callback=inginious_container_api.run_student._hack_signals, ssh=True, run_as_root=run_as_root, closure_script=closure_script)
