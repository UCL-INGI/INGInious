Installation and deployment on Ubuntu with Apache
=================================================

This page explains how to install INGInious on Ubuntu 16.04--18.04, with an Apache 2 server.
...
Add the `www-data` user in the necessary groups, to allow it to launch new containers and to connect to mongodb:
::

    # sermod -aG docker www-data
    # usermod -aG mongodb www-data

Create a folder for INGInious in the /var/www folder:
::
...
.. note::
   ...
   ::
       ...
       $ pip3 install --upgrade git+https://github.com/UCL-INGI/INGInious.git@v0.6#egg=INGInious[ldap]
...
To configure the web app automatically, go to the inginious folder, create folders for tasks and backup, then use the ``inginious-install`` CLI.
::
...
When asked about the tasks folder, enter an absolute folder: /var/www/inginiuos/tasks, and similarly for backup.
...
In the former case, the frontend will automatically start a local backend and grading agents.
