...
def ressource(request):
    ...
    taskset_factory, _ = create_factories(fs, task_dispensers, problem_types)
    yield ( taskset_factory)
...
class TestTaskBasic(object):
    ...
    def test_task_loading(self, ressource):
        ...
        taskset_factory = ressource
        print("\033[1m-> common-tasks: task loading\033[0m")
        t = taskset_factory.get_task('test', 'task1')
    ...
    def test_task_invalid_name(self, ressource):
        taskset_factory = ressource
        try:
            taskset_factory.get_task('test', 'invalid/name')
    ...
    def test_task_invalid(self, ressource):
        taskset_factory = ressource
        try:
            taskset_factory.get_task('test3', 'invalid_task')
    ...
    def test_no_problems(self, ressource):
        taskset_factory = ressource
        try:
            Task(taskset_factory.get_course('test3'), 'invalid_task',
    ...
    def test_input_consistent_valid(self, ressource):
        taskset_factory = ressource
        c = taskset_factory.get_course("test")
        t = c.get_task("task3")
...
class TestTaskProblem(object):
    def test_problem_types(self, ressource):
        ...
        t = taskset_factory.get_task('test2', 'task1')
        assert t.get_problems()[0].get_type() == 'match'

        t = taskset_factory.get_task('test2', 'task2')
        assert t.get_problems()[0].get_type() == 'match'

        t = taskset_factory.get_task('test2', 'task3')
        assert t.get_problems()[0].get_type() == 'multiple_choice'
    ...
    def test_multiple_choice(self, ressource):
        ...
        p = taskset_factory.get_task('test2', 'task3').get_problems()[0]
    ...
    def test_match(self, ressource):
        ...
        p = taskset_factory.get_task('test2', 'task1').get_problems()[0]
    ...
    def test_code(self, ressource):
        ...
        p = taskset_factory.get_task('test', 'task3').get_problems()[0]
    ...
    def test_file(self, ressource):
        ...
        p = taskset_factory.get_task('test2', 'task4').get_problems()[0]
