...
def ressource(request):
    ...
    taskset_factory.update_course_descriptor_content("test", {"name": "Unit test 1", "admins": ["testadmin1","testadmin2"],
                                                             "accessible": True})
...
class TestCourse(object):
    ...
    def test_course_loading(self, ressource):
        ...
        c = taskset_factory.get_course('test2')
        ...
        # This one is in JSON
        c = taskset_factory.get_course('test3')
    ...
    def test_invalid_coursename(self, ressource):
        try:
            taskset_factory, temp_dir = ressource
            taskset_factory.get_course('invalid/name')
        except:
            return
        assert False
    ...
    def test_unreadable_course(self, ressource):
        try:
            taskset_factory, temp_dir = ressource
            taskset_factory.get_course('invalid_course')
        except:
            return
        assert False
    ...
    def test_all_courses_loading(self, ressource):
        ...
        taskset_factory, temp_dir = ressource
        c = taskset_factory.get_all_courses()
        assert 'test' in c
        assert 'test2' in c
        assert 'test3' in c
        ...
        with open(os.path.join(temp_dir, "test", "course.yaml"), "w") as f:
            f.write("""
                ...
                accessible: "1970-01-01/2033-01-01"
                        """)
        assert dict(taskset_factory.get_course_descriptor_content("test")) != {"name": "a", "admins": ["a"],
                                                                                    "accessible": "1970-01-01/2033-01-01"}
        taskset_factory.update_course_descriptor_content("test", {"name": "b", "admins": ["b"],
                                                                 "accessible": "1970-01-01/2030-01-01"})
        assert dict(taskset_factory.get_course_descriptor_content("test")) == {"name": "b", "admins": ["b"],
                                                                              "accessible": "1970-01-01/2030-01-01"}
