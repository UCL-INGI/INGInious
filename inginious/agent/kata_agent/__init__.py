...
import msgpack
import psutil
from inginious.agent.kata_agent._kata_interface import KataInterface

from inginious.agent import Agent, CannotCreateJobException
from inginious.agent.kata_agent._timeout_watcher import TimeoutWatcher
...
class KataAgent(Agent):
    def __init__(self, context, backend_addr, friendly_name, concurrency, tasks_fs: FileSystemProvider, address_host=None, external_ports=None, tmp_dir="./agent_tmp"):
        ...
        """
        super(KataAgent, self).__init__(context, backend_addr, friendly_name, concurrency, tasks_fs)
        self._logger = logging.getLogger("inginious.agent.kata")
    ...
    async def _init_clean(self):
        ...
        # Docker
        self._docker = AsyncProxy(KataInterface())
        ...
        for idx in self._containers:
            self._containers[idx]["type"] = "kata" # type is not given by self._docker.get_containers()
    ...
    async def run(self):
        ...
        try:
            await super(KataAgent, self).run()
        except:
