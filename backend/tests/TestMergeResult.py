...
class TestMerge(TestWithFakeAgent):
    ...
    def test_merge_text(self):
        ...
        self.wait_for_callback()
        assert "result" in self.got_result and self.got_result["result"] == "success"
        assert "problems" in self.got_result
        assert "problem_1" in self.got_result["problems"] and "problem_2" in self.got_result["problems"]
        assert self.got_result["problems"]["problem_1"] == "a"
        assert self.got_result["problems"]["problem_2"] == "Correct answer"
    ...
    def test_grade_no_run_success(self):
        ...
        self.job_manager.new_job(Course('test').get_task('no_run'), {"problem_id": "1"}, self.default_callback)
        self.wait_for_callback()
        assert "grade" in self.got_result and self.got_result["grade"] == 100.0
    ...
    def test_grade_no_run_failed(self):
        ...
        self.wait_for_callback()
        assert "grade" in self.got_result and self.got_result["grade"] == 0
