...
class CourseGroupListPage(object):
    ...
    def page(self, course):
        ...
        groups = list(get_database().groups.find({"course_id": course.get_id()}))
        groups = [group for group in groups if len(group["users"]) > 0]

        # Group statistics corresponds to one of its member's statistics
        search_users = [group["users"][0] for group in groups]
        ...
        data = [dict(data[user].items() +
                     [("url", self.submission_url_generator(course, str(group["_id"]))),
                      ("first_user", user), ("description", group["description"]), ("group_id", group["_id"])])
                for group, user in zip(groups, search_users) if user in data]

        if "csv" in web.input():
            return make_csv(data)
        return renderer.course_admin.group_list(course, data)
