...
from frontend.webapp.pages.api._api_page import APIPage
...
class APIAuthMethods(APIPage):
    ...
    def API_GET(self):
        ...
        """
        to_display = []
        for key, (name, method_input) in self.user_manager.get_auth_methods_inputs().iteritems():
            to_display.append({
                "id": key,
                "name": name,
                "input": [{"id": ik, "name": iv["placeholder"], "type": iv["type"]} for ik, iv in method_input.iteritems()]
            })
